...
"""
    cb_get(cb_data, cb_where)

Fetch callback node status and load callback primal values.

Returns:
- Result of `Gurobi.load_callback_variable_primal`.
"""
function cb_get(cb_data, cb_where)
    resultP = Ref{Cint}()
...
"""
    cb_retrieve_V0_openbranches(cb_data, m::Model, g::MetaGraph)

Retrieve rounded branch statuses and open-branch set from callback values.

Returns:
- Named tuple `(v_0, openbranches)`.
"""
function cb_retrieve_V0_openbranches(cb_data, m::Model, g::MetaGraph)
    v_0 = Dict{ELabel,Float64}()
    ...
    (v_0 = v_0, openbranches = openbranches)
end

"""
    cb_retrieve_openbranches(cb_data, m::Model, g::MetaGraph)

Retrieve currently open branches at callback point.

Returns:
- `Vector{ELabel}`.
"""
function cb_retrieve_openbranches(cb_data, m::Model, g::MetaGraph)
    openbranches = Vector{ELabel}()
    for br in edge_labels(g)
        round(callback_value(cb_data, m[:v][br...])) == 0.0 && push!(openbranches, br)
    end
    openbranches
end

"""
    cb_pocket_π_to_0!(cb_data, cb_where, m::Model, ec::ElementaryCase, contingencies::Vector{ELabel}; rounddigits=5)

Generate lazy cuts enforcing pocket de-energization consistency in callback.

Returns:
- `Nothing`.

Side effects:
- Submits lazy constraints to `m`.
"""
function cb_pocket_π_to_0!(
    cb_data,
    cb_where,
    m::Model,
    ec::ElementaryCase,
    contingencies::Vector{ELabel};
    rounddigits = 5,
)
    g = ec.g
    cb_get(cb_data, cb_where)

    openbranches = cb_retrieve_openbranches(cb_data, m, g)
    _log = "\topen $(length(openbranches)) branches: $openbranches\n"

    bridge_to_pocket = create_bridge_to_pocket(ec, openbranches)
    newcuts = 0

    for cbr in contingencies
        !(cbr in keys(bridge_to_pocket)) && continue
        (busfrom, busto) = cbr
        c = branch_to_case(cbr)
        πfrom, πto =
            round(callback_value(cb_data, m[:π][c, busfrom]); digits = rounddigits),
            round(callback_value(cb_data, m[:π][c, busto]); digits = rounddigits)
        if πfrom + πto == 1
            continue
        end
        pk = bridge_to_pocket[cbr]
        con = @build_constraint(
            sum(m[:π][branch_to_case(cbr), bus] for bus in pk.buses) ≤
            length(pk.buses) * (sum(m[:v][br...] for br in pk.branches))
        )
        MOI.submit(m, MOI.LazyConstraint(cb_data), con)
        newcuts += 1
    end
    _log *= "\tNew πcuts cuts added: $newcuts\n"
end

"""
    cb_generate_optimality_cuts!(m::Model, cb_data, contingency::ELabel, bridge_to_pocket::Dict{ELabel,Pocket}, structural_bridges=ELabel[])

Generate lazy optimality cuts for one violated contingency pocket.
...
- Submits lazy constraints to `m`.
"""
function cb_generate_optimality_cuts!(
    m::Model,
    cb_data,
    contingency::ELabel,
    bridge_to_pocket::Dict{ELabel,Pocket},
    structural_bridges = ELabel[],
)
    nbcuts = 0
    !(contingency in keys(bridge_to_pocket)) && return 0
    contingency in structural_bridges && return 0 # structural_bridges already in the master problem that otherwise would be redundantly repeated for each callback call
    pk = bridge_to_pocket[contingency]
    pk.d ≤ 0 && return 0
    branches = union(pk.branches, [contingency])
    for open_br in branches
        con = @build_constraint(
            m[:lostload][open_br...] ≥
            (1 - sum(m[:v][br...] for br in branches if br ≠ open_br)) * pk.d
        )
        MOI.submit(m, MOI.LazyConstraint(cb_data), con)
        nbcuts += 1
    end
...
"""
    check_feasibility_cut(g, dual_obj, rc, v_0, open)

Check whether a reduced-cost feasibility cut is satisfied for a candidate opening set.

Returns:
- `Bool`.
"""
check_feasibility_cut(g, dual_obj, rc, v_0, open) =
    dual_obj + sum(rc[br] * (1 - (br in open) - v_0[br]) for br in edge_labels(g)) ≤ 0

"""
    cb_generate_extra_feasible_cuts_leveraging_zeros!(m, g, cb_data, openbranches, contingency, reduced_cost; atol=1e-6, e_neighborhood=nothing, openings_to_test=nothing)

Generate additional feasibility cuts by exploiting near-zero reduced-cost coefficients.
...
- Submits lazy constraints to `m`.
"""
function cb_generate_extra_feasible_cuts_leveraging_zeros!(
    m,
    g,
    cb_data,
    openbranches,
    ...
    reduced_cost;
    atol = 1e-6,
    e_neighborhood = nothing,
    openings_to_test::Union{Nothing,Vector{ELabel}} = nothing,
)
    nb = 0
    null_coefs = [
        br for br in edge_labels(g) if (
            isapprox(reduced_cost[br...], 0; atol = atol) ||
            e_neighborhood ≠ nothing && !(br in e_neighborhood[contingency])
        )
    ]
    deleteat!(null_coefs, findall(br -> br == contingency, null_coefs))
    _openbranches = [openbranches; contingency]

    open_wo_null = [br for br in _openbranches if !(br in null_coefs)]
    open_or_null =
        [br for br in edge_labels(g) if (br in null_coefs || br in _openbranches)]
    for nonnull in open_wo_null
        con = @build_constraint(
            sum((m[:v][br...]) for br in open_wo_null if br ≠ nonnull) +
            sum((1 - m[:v][br...]) for br in edge_labels(g) if !(br in open_or_null)) ≥
            1
        )
        MOI.submit(m, MOI.LazyConstraint(cb_data), con)

        if !isnothing(openings_to_test)
            s =
                (isempty(open_wo_null) || open_wo_null == [nonnull]) ? 0 :
                sum(!(br in openings_to_test) for br in open_wo_null if br ≠ nonnull)
            s +=
                (length(open_or_null) == ne(g)) ? 0 :
                sum(
                    br in openings_to_test for br in edge_labels(g) if !(br in open_or_null)
                )
            s == 0 &&
                @warn "extra feasibility cut violated by openings_to_test: openbranches:$openbranches\n\tcontingency:$contingency,\n\tnonnull=$nonnull, \n\topen_wo_null=$open_wo_null\n\t"
        end
    end
    nb = length(open_wo_null)
    nb
end
